...
def extract_code(reply: str, prompt: str) -> str:
    ...
    5. Nothing useful left → return prompt + pass.
    """
    ...
    # Step 4 — if nothing useful remains, return a stub so it fails cleanly
...
def run_humaneval(base_url: str, model: str) -> dict:
    ...
    for i, item in enumerate(HUMANEVAL, 1):
        ...
        reply = chat(base_url, model, messages, max_tokens=1024)
        if reply is None:
